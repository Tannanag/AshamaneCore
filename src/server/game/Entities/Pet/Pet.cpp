...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    switch (petType)
    {
        case SUMMON_PET:
        {
            ...
            int32 fire = GetOwner()->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_FIRE);
            int32 shadow = GetOwner()->GetUInt32Value(AsUnderlyingType(PLAYER_FIELD_MOD_DAMAGE_DONE_POS) + SPELL_SCHOOL_SHADOW);
