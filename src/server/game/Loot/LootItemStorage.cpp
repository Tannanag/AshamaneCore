...
NeedsQuest(lootItem.needs_quest), RandomPropertyId(lootItem.randomPropertyId.Id), Context(static_cast<ItemContext>(lootItem.context)), BonusListIDs(lootItem.BonusListIDs)
{
}
...
void LootItemStorage::LoadStorageFromDB()
{
    ...
    if (result)
    {
        do
        {
            ...
            lootItem.randomPropertyId.Id = fields[9].GetInt32();
...
bool LootItemStorage::LoadStoredLoot(Item* item, Player* player)
{
    ...
    if (LootTemplate const* lt = LootTemplates_Item.GetLootFor(item->GetEntry()))
    {
        for (auto const& storedItemPair : container->GetLootItems())
        {
            ...
            li.randomPropertyId.Id = storedItemPair.second.RandomPropertyId;
...
void StoredLootContainer::AddLootItem(LootItem const& lootItem, CharacterDatabaseTransaction& trans)
{
    ...
    stmt->setInt32(9, lootItem.randomPropertyId.Id);
