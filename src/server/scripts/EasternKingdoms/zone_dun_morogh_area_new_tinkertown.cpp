...
enum SafeOperativeSparring
{
    ...
    // 0 to 40.
    //
    // Its SpellVisual 18304 used to leave a foreign gun on the caster: of its three
    // kits, 17337 exists only to attach SpellVisualEffectName 3113, a model no item
    // in the game uses, and at this firing rate that replaced the equipped 52355 and
    // outlived the fight. Kit 17337 is now deleted client-side by a hotfix_data row
    // against SpellVisualKit, so 18304 plays as its other two kits plus its own
    // missile. See sql/ashamane/hotfixes/2026_08_28_00_hotfixes.sql.
    SPELL_SHOOT            = 85756
};
