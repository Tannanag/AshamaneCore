...
class spell_immerseus_sha_pool_dmg : public SpellScript
{
    ...
    {
        p_Targets.remove_if([&](WorldObject* p_Target) -> bool { return GetCaster()->GetExactDist2d(p_Target) > (0.6f * GetCaster()->GetFloatValue(OBJECT_FIELD_SCALE_X)); });
    }
...
class spell_immerseus_sha_pool_script : public SpellScript
{
    ...
    void ScaleRange(std::list<WorldObject*>& p_Targets)
    {
        ...
        {
            return !p_Target->IsPlayer() || GetCaster()->GetExactDist2d(p_Target) > (0.6f * GetCaster()->GetFloatValue(OBJECT_FIELD_SCALE_X));
        });
    }
