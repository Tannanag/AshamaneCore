...
private:
    ...
    void MakeDied()
    {
        ...
        // Do it manually
        me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_UNK_29);                                                // blizz like 2.0.x
        me->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_FEIGN_DEATH);                                              // blizz like 2.0.x
        me->SetFlag(CORPSE_FIELD_DYNAMIC_FLAGS, UNIT_DYNFLAG_DEAD);
    ...
    public:
        ...
        struct npc_paragons_of_the_klaxxi_mantid_amberAI : public Scripted_NoMovementAI
        {
            npc_paragons_of_the_klaxxi_mantid_amberAI(Creature* creature) : Scripted_NoMovementAI(creature)
            {
                ...
                me->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG_NOT_SELECTABLE);
            }
    ...
    public:
        ...
        class spell_paragons_of_the_klaxxi_devour_AuraScript : public AuraScript
        {
            ...
            void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                GetUnitOwner()->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED);
            ...
            void HandleRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                GetUnitOwner()->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED);
...
class spell_paragons_of_the_klaxxi_fire : public SpellScriptLoader
{
    public:
        spell_paragons_of_the_klaxxi_fire() : SpellScriptLoader("spell_paragons_of_the_klaxxi_fire") { }

        class spell_paragons_of_the_klaxxi_fire_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_paragons_of_the_klaxxi_fire_SpellScript);

            void FilterTarget(WorldObject*& target)
            {
                m_MainTargetGuid = target->GetGUID();
            }
            ...
            void FilterTargets(std::list<WorldObject*> &targets)
            {
                ...
                Unit* mainTarget = ObjectAccessor::GetUnit(*caster, m_MainTargetGuid);
                if (!mainTarget)
                {
                    targets.clear();
                    return;
                ...
                {
                    return !obj->IsPlayer() ||
                        (obj->GetGUID() != mainTarget->GetGUID() &&
                        !obj->IsInBetween(caster, mainTarget, 2.0f));
                });
            }

            void Register()
            {
                OnObjectTargetSelect += SpellObjectTargetSelectFn(spell_paragons_of_the_klaxxi_fire_SpellScript::FilterTarget, EFFECT_0, TARGET_UNIT_TARGET_ANY);
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_paragons_of_the_klaxxi_fire_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_CONE_ENEMY_24);
            }
        ...
        SpellScript* GetSpellScript() const
        {
            return new spell_paragons_of_the_klaxxi_fire_SpellScript();
        }
};
        ...
        private:
            ...
            void ApplyHurlAmberSpell(Player* player, bool value)
            {
                if (value)
                {
                    ...
                    //player->SetUInt16Value(PLAYER_FIELD_OVERRIDE_SPELLS_ID, 1, 456);
                    player->SetUInt16Value(PLAYER_FIELD_BYTES_3_OFFSET_OVERRIDE_SPELLS_ID, 1, 456);
                    player->AddTemporarySpell(SPELL_HURL_AMBER_MISSILE_2);
                }
                else
                {
                    m_IsHurlAmberSpellApplied = false;
                    //player->SetUInt16Value(PLAYER_FIELD_OVERRIDE_SPELLS_ID, 1, 455);
                    player->SetUInt16Value(PLAYER_FIELD_BYTES_3_OFFSET_OVERRIDE_SPELLS_ID, 1, 455);
                    player->RemoveTemporarySpell(SPELL_HURL_AMBER_MISSILE_2);
                }
            }
...
struct spell_area_paragons_of_the_klaxxi_sonic_projection : AreaTriggerAI
{
    ...
    void OnCreate() override
    {
        ...
        float targetX = 0.0f;
        float targetY = 0.0f;
        caster->GetNearPoint2D(targetX, targetY, 60.0f, frand(0.0f, 2 * M_PI));
...
struct spel_area_paragons_of_the_klaxxi_sonic_pulse : AreaTriggerAI
{
    ...
    void OnCreate() override
    {
        ...
        const float sector = 2 * M_PI / (SONIC_PULSE_SPELLS_COUNT * SONIC_PULSE_COUNT);
...
void AddSC_boss_paragons_of_the_klaxxi()
{
    ...
    new spell_paragons_of_the_klaxxi_fire();            // 142950
